...
export default function WhatWeDoIndex() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      // Fix: Use 'service_id' instead of 'services_id'
      const res = await api.get(`/what-we-do?service_id=${serviceId}`);
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">
            What We Do {serviceName && `- ${serviceName}`}
          </h1>
          <p className="text-sm text-gray-600 mt-1">
            Manage services and details for {serviceName || "your offering"}
          </p>
        </div>

        <Button
          onClick={() =>
            navigate(`/admin/dashboard/services/${serviceId}/what-we-do/create`)
          }
                ...
                >
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <Button
                      onClick={() =>
                        navigate(
                          `/admin/dashboard/services/${serviceId}/what-we-do/create`,
                        )
                      }
                    ...
                    <div className="flex flex-wrap gap-1">
                      {item.services?.length > 0 ? (
                        item.services?.slice(0, 3).map((s, idx) => (
                          <span
                            key={idx}
                            className="text-xs bg-gray-100 px-2 py-1 rounded-none"
                            title={s}
                          >
                      ...
                      {item.services?.length > 3 && (
                        <span className="text-xs bg-gray-100 px-2 py-1 rounded-none">
                  ...
                  <TableCell className="py-4 pl-4 pr-4">
                    <div className="flex justify-end items-center gap-2">
                      ...
                      <ActionButton
                        ...
                        onClick={() =>
                          navigate(
                            `/admin/dashboard/services/${serviceId}/what-we-do/${item.id}`,
                          )
                        }
                      />
                      ...
                      <ActionButton
                        ...
                        onClick={() =>
                          navigate(
                            `/admin/dashboard/services/${serviceId}/what-we-do/${item.id}/edit`,
                          )
                        }
                      />
