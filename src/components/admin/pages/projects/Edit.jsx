...
export default function EditProject() {
  ...
  const handleSubmit = async (e) => {
    ...
    form.existingImages.forEach((img) => {
      formData.append("existing_images[]", img.id || img.image_path);
    });
    ...
    try {
      await api.post(`/projects/${id}?_method=PUT`, formData, {
  ...
  if (loadingData) {
    return (
      <div className="max-w-4xl mx-auto bg-white border border-gray-200 p-8">
        <div className="text-gray-500 text-base">Loading project data...</div>
      </div>
    );
  }
